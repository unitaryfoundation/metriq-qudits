...
def _calibrate_buffers(d, depth, targets, seed, penalty_weight, stability_th, n_jobs):
    ...
    for num_buffers in range(MIN_BUFFERS, MAX_BUFFERS + 1):
        ...
        stabilities = [
            float(np.nanmax(stability_infidelity(
                circuit, targets[i], [n_cavity + e for e in N_TEST_EXTRA], d, 1)))
            for i, circuit in enumerate(compiled) if circuit is not None
        ]
        if stabilities and max(stabilities) < stability_th:
            return num_buffers, n_penalize
    return MAX_BUFFERS, _buffer_count_to_penalize(MAX_BUFFERS)
...
def calibrate(d: int, *, n_jobs: int, penalty_weight: float = PENALTY_WEIGHT,
    ...
    num_buffers, n_penalize = _calibrate_buffers(
    ...
    k_start = _probe_start_depth(
        ...
        penalty_weight, stability_th, n_jobs)
    return Calibration(n_cavity=n_cavity, n_penalize=n_penalize,
                       k_start=k_start, k_max=K_MAX_FACTOR * depth)


def _write(path, cal: Calibration) -> None:
    np.savez(str(path), n_cavity=cal.n_cavity, n_penalize=cal.n_penalize,
...
def load_or_calibrate(path, d: int, *, overwrite: bool = False, n_jobs: int,
    ...
    cal = calibrate(d, n_jobs=n_jobs, penalty_weight=penalty_weight, stability_th=stability_th)
    _write(path, cal)
    return cal
