...
def make_storages(num_modes: int) -> list[Storage]:
    return [
        Storage(chi_kHz=CHI_KHZ, chi_prime_Hz=CHI_PRIME_HZ, Ks_Hz=SELF_KERR_HZ)
        for _ in range(num_modes)
    ]


def make_qubit() -> Qubit:
    return Qubit(sigma_ns=QUBIT_SIGMA_NS, chop=QUBIT_CHOP)
    ...
    *,
    ...
    if os.path.exists(output_path) and not overwrite:
        ...
        settings_match = (
            ...
            == QUBIT_SIGMA_NS
            and int(cached_metadata.get("qubit_chop", -1)) == QUBIT_CHOP
        )
        if settings_match:
...
def save_gate_trajectory_diagnostics(compiled_path: str, output_dir: str) -> None:
    ...
    for circuit_index, circuit in enumerate(compiled):
        ...
        gate = builder.ecd_gate(beta=beta, alpha=PEAK_TRANSIENT_DISPLACEMENT)
        alpha_g, alpha_e = builder._ecd_trajectory(
